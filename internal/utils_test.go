...
`,
		...
		err := ioutil.WriteFile(manifestPath, []byte(manifestYAML), 0666)
		if err != nil {
	...
	err := ioutil.WriteFile(bogusFilePath, []byte("# My Manifests"), 0666)
	if err != nil {
	...
	for _, test := range tests {
		...
		objTemplates := spec["object-templates"].([]map[string]interface{})
		assertEqual(t, len(objTemplates), 2)
		assertEqual(t, objTemplates[0]["complianceType"], "musthave")
		assertEqual(
			t, objTemplates[0]["objectDefinition"].(map[string]interface{})["kind"], "ConfigMap",
		)
		assertEqual(t, objTemplates[1]["complianceType"], "musthave")
		assertEqual(
			t, objTemplates[1]["objectDefinition"].(map[string]interface{})["kind"], "ConfigMap",
		)
	}
}
...
func TestGetPolicyTemplateInvalidManifest(t *testing.T) {
	...
	err := ioutil.WriteFile(manifestPath, []byte("$i am not YAML!"), 0666)
	if err != nil {
		t.Fatalf("Failed to write %s", manifestPath)
	}
...
data:
  game.properties: |
    enemies=potato
`
	err := ioutil.WriteFile(manifestsPath, []byte(yamlContent), 0666)
	if err != nil {
...
func TestUnmarshalManifestFileInvalidYAML(t *testing.T) {
	...
	err := ioutil.WriteFile(manifestPath, []byte(yamlContent), 0666)
	if err != nil {
		t.Fatalf("Failed to write %s", manifestPath)
	}
